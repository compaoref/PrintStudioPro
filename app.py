...
with tab_vinyl:
    ...
    # ── PARAMÈTRES ──────────────────────────────────────────
    st.markdown('<div class="section-title">📐 Dimensions & Multiplication</div>', unsafe_allow_html=True)

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        print_w = st.number_input("Largeur totale (m)", min_value=0.01, max_value=10.0, value=1.50, step=0.01, format="%.2f")
    with c2:
        print_h = st.number_input("Hauteur totale (m)", min_value=0.01, max_value=10.0, value=1.00, step=0.01, format="%.2f")
    with c3:
        cols_v = st.number_input("Colonnes (X)", min_value=1, max_value=100, value=3, step=1)
    with c4:
        rows_v = st.number_input("Lignes (Y)", min_value=1, max_value=100, value=4, step=1)

    c5, c6, c7, c8 = st.columns(4)
    with c5:
        gap_h = st.number_input("Espacement H (mm)", min_value=0.0, max_value=100.0, value=3.0, step=0.5)
    with c6:
        gap_v = st.number_input("Espacement V (mm)", min_value=0.0, max_value=100.0, value=3.0, step=0.5)
    with c7:
        margin_v = st.number_input("Marge bord (mm)", min_value=0.0, max_value=100.0, value=5.0, step=1.0)
    with c8:
        bleed_v = st.number_input("Fond perdu (mm)", min_value=0.0, max_value=20.0, value=3.0, step=0.5)
    ...
    with cb:
        rotation_v = st.selectbox("Rotation étiquette", [0, 90, 180, 270], index=0, format_func=lambda x: f"{x}°")
    with cc:
    ...
    output_mode_v = st.radio(
        ...
        format_func=lambda x: {
            "tif_only":    "🖼️  TIF CMJN uniquement (téléchargement direct)",
            "pmn_only":    "🖨️  PMN uniquement (job RIP pour MainTap)",
            "tif_and_pmn": "📦  TIF + PMN ensemble (ZIP)",
    ...
    m_px   = px_from_mm(margin_v, dpi_v)
    gh_px  = px_from_mm(gap_h, dpi_v)
    gv_px  = px_from_mm(gap_v, dpi_v)
    cw_est = max(1, (wpx_est - 2*m_px - gh_px*(cols_v-1)) // cols_v)
    ch_est = max(1, (hpx_est - 2*m_px - gv_px*(rows_v-1)) // rows_v)
    cw_mm  = (cw_est / dpi_v) * MM_PER_INCH
    ch_mm  = (ch_est / dpi_v) * MM_PER_INCH

    st.markdown('<div class="section-title">📊 Récapitulatif</div>', unsafe_allow_html=True)
    s1,s2,s3,s4,s5 = st.columns(5)
    s1.markdown(f'<div class="stat-card"><div class="stat-label">Étiquettes</div><div class="stat-value orange">{total_labels}</div></div>', unsafe_allow_html=True)
    s2.markdown(f'<div class="stat-card"><div class="stat-label">Taille / étiquette</div><div class="stat-value">{cw_mm:.0f} × {ch_mm:.0f} mm</div></div>', unsafe_allow_html=True)
    s3.markdown(f'<div class="stat-card"><div class="stat-label">Planche finale</div><div class="stat-value">{print_w:.2f} × {print_h:.2f} m</div></div>', unsafe_allow_html=True)
    ...
    s5.markdown(f'<div class="stat-card"><div class="stat-label">Taille TIF estimée</div><div class="stat-value green">{estimate_size(wpx_est,hpx_est,4)}</div></div>', unsafe_allow_html=True)

    # ── APERÇU ───────────────────────────────────────────────
    st.markdown('<div class="section-title">👁️ Aperçu Disposition</div>', unsafe_allow_html=True)

    src_img_v = None
    if up_vinyl:
        try:
            if up_vinyl.name.lower().endswith(".pdf") and not HAS_MUPDF:
                st.warning("PDF chargé — PyMuPDF non disponible pour l'aperçu. Le traitement fonctionnera quand même.")
            else:
    ...
    st.image(preview_v, use_container_width=True, caption=f"Disposition {int(cols_v)}×{int(rows_v)} — {int(cols_v)*int(rows_v)} étiquettes — {print_w:.2f}m×{print_h:.2f}m")
    ...
    if not up_vinyl:
        ...
        btn_rip_v = st.button(
            "🖨️  Lancer RIP — Générer fichier prêt à imprimer",
            type="primary",
        ...
        if btn_rip_v:
            try:
                ...
                tif_bytes, info = rip_grand_format(
                    ...
                    print_w_m=float(print_w), print_h_m=float(print_h),
                ...
                fname_tif = f"{base_name}_{print_w:.2f}mx{print_h:.2f}m_{int(cols_v)}x{int(rows_v)}_{dpi_v}dpi_CMJN_{ts}.tif"
                fname_pmn = fname_tif.replace(".tif", ".pmn")

                job_info = {
                    "name": base_name,
                    "media_w_mm": print_w * 1000, "media_h_mm": print_h * 1000,
                    "orientation": "landscape" if print_w > print_h else "portrait",
                    "copies": 1, "dpi": dpi_v, "icc": icc_v,
                    "mirror": False, "white_base": False,
                    "bleed_mm": bleed_v,
                    "cols": int(cols_v), "rows": int(rows_v),
                    "gap_h": gap_h, "gap_v": gap_v, "margin": margin_v,
                    "source_file": up_vinyl.name, "mode": "GrandFormat",
                    "quality": "High",
                }
